"""
Firewall Rule Checker - Specific Source/Destination with Any Service Analysis
Analyzes firewall rules with specific source and destination but permissive service settings.
...
class SourceDestSpecificAnalyzer:
    """Analyzes firewall rules for specific sources and destinations with permissive service settings."""
    
    def __init__(self):
        self.setup_logging()
        self.base_dir = Path(__file__).parent
        self.firewall_path = self.base_dir / 'modified_firewall_updated.xlsx'
        self.output_path = self.base_dir / 'output_Source-Specific--Destination-Specific--Services-Any.xlsx'
    ...
    def setup_logging(self) -> None:
        ...
        log_file = log_dir / f"source_dest_specific_analyzer_{timestamp}.log"
        ...
        self.logger = logging.getLogger("SourceDestSpecificAnalyzer")
    ...
    def analyze_rule(self, row: pd.Series, row_num: int) -> Optional[Dict]:
        ...
        try:
            ...
            # 2. Destination must be specific (non-any)
            # 3. Service must be 'any'
            if (self.has_specific_value(source) and 
                self.has_specific_value(destination) and 
                self.is_all_any(service)):
                return {
                    "Row Number": row_num,
                    ...
                    "Service": service,
                    "Risk Level": "Medium",
                    "Finding": "Rule allows all services between specific endpoints",
                    "Recommendation": "Review and restrict services to required protocols"
                }
    ...
    def analyze_rules(self, rules_df: pd.DataFrame) -> List[Dict]:
        """Analyze rules for specific source/destination with any service patterns."""
        findings = []
        ...
        self.logger.info(f"Analysis complete. Found {len(findings)} rules with specific endpoints but any service.")
        return findings
    ...
    def display_findings(self, findings: List[Dict]) -> None:
        ...
        if findings:
            ...
            print("\nRecommendations:")
            print("1. Review all rules allowing unrestricted services")
            print("2. Document required services for each communication path")
            print("3. Implement service-specific rules based on business needs")
            print("4. Consider implementing application-layer filtering where appropriate")
        else:
...
def main():
    ...
    try:
        analyzer = SourceDestSpecificAnalyzer()
