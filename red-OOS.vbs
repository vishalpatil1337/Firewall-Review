...
' File paths
txtFilePath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\OOS.txt"
...
lastRow = objWorksheet.Cells(objWorksheet.Rows.Count, "D").End(-4162).Row
Set rngD = objWorksheet.Range("D1:D" & lastRow)
...
If fso.FileExists(txtFilePath) Then
    ...
    txtFile.Close
Else
    WScript.Echo "OOS.txt not found at: " & txtFilePath
    objWorkbook.Close False
    objExcel.Quit
    WScript.Quit
End If
...
' Loop through the selected range and highlight matches in red
For Each cell In rngC
    ...
    For Each ip In Split(ipList, "|")
        ...
        End If
    Next
    If matchFound Then cell.Interior.Color = RGB(255, 0, 0) ' Red
Next

' Loop through column D and highlight matches in red
